...
INNER JOIN {{ source('vocabularies', 'concept_ancestor') }} ca ON ca.DESCENDANT_CONCEPT_ID = d.DRUG_CONCEPT_ID
INNER JOIN {{ source('vocabularies', 'concept') }} c ON ca.ANCESTOR_CONCEPT_ID = c.CONCEPT_ID
